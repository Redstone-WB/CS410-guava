...
def parse_to_sentence_UseVocab(content, Vocab, VocabDict):
    ...
    for sent in sentences:
        ...
        temp = [stemmer.stem(w.lower())
                for w in words if stemmer.stem(w.lower()) in Vocab]
...
class Restaurant:
    def __init__(self, rest_data, Vocab, VocabDict):
        ...
        self.Reviews = [Review(review, Vocab, VocabDict) for review in rest_data.get(
...
# one sent and all aspect terms, return hit counts for each aspect term
def sent_aspect_match(stn, aspects):
...
class Corpus:
    def __init__(self, corpus, Vocab, Count, VocabDict):
        ...
        self.V = len(Vocab)
        self.Aspect_Terms = []
        self.Restaurants = [Restaurant(rest, Vocab, VocabDict)
                            for rest in corpus]
...
class Bootstrapping:
    ...
    # corpu.Restaurants > review > Stn (sentence)
            ...
            for rest in corpus.Restaurants:
                for review in rest.Reviews:
                    for stn in review.Stns:
                        ...
                        if max(count) > 0:
                            s_label = np.where(np.max(count) == count)[
                                0].tolist()
    ...
    def calc_chi_sq(self, corpus):
        ...
        if K > 0:
            for rest in corpus.Restaurants:
                for review in rest.Reviews:
                    collect_stat_for_each_review(
                        review, self.Aspect_Terms, corpus.Vocab)
                    ...
                    for w in review.UniWord:
                        # index, since the matrix for review is small
                        z = np.where(w == review.UniWord)[0][0]
                        corpus.all_num_stn_word[w] = corpus.all_num_stn_word[w] + \
                            review.num_stn_word[z]
                        corpus.all_num_stn_aspect_word[:, w] = corpus.all_num_stn_aspect_word[:,
                                                                                              w] + review.num_stn_aspect_word[:, z]

            for k in range(K):
                for w in range(V):
                    Chi_sq[k, w] = ChisqTest(corpus.all_num_stn, corpus.all_num_stn_aspect_word[k, w],
                                             corpus.all_num_stn_word[w], corpus.all_num_stn_aspect[k])
            self.Chi_sq = Chi_sq
        else:
...
def create_W_matrix_for_each_review(analyzer, review, corpus):
    ...
    # V=len(corpus.Vocab)
    review.W = np.zeros((K, Nd))  # 각 Review의 단어들에 대해서 Aspect Term (Topic)에 대한
    for k in range(K):
        for w in range(Nd):  # w is index of UniWord_for_review
            # z = review.UniWord[w]
            sum_row = sum(review.num_stn_aspect_word[k])
            if sum_row > 0:
                review.W[k, w] = review.num_stn_aspect_word[k, w] / \
                    sum_row  # 행합으로 각 단어 frequency를 나눠 줌
...
def produce_data_for_rating(analyzer, corpus, outputfolderpath):
    ...
    f = open(vocabfile, "w")
    for w in corpus.Vocab:  # corpus (data.Vocab에 저장된) 의 모든 단어를 write
        f.write(w+",")
    f.close()
...
def print_summary_stats(corpus):
    ...
    print("TotalNumOfAnnotatedReviews =" +
          str(TotalNumOfAnnotatedReviews) + "\n")
